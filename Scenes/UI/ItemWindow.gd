...
func present_bag(player_index: int) -> void:
	...
	for ball in inv.balls.keys():
		_body.add_child(_named_with_desc(
			"%s ×%d" % [_ball_name(ball), inv.balls[ball]], _ball_desc(ball)))
...
func _item_use_row(entry: Dictionary) -> Control:
	...
	box.add_child(row)
	box.add_child(_desc_label(_describe(entry)))
	return box

# A plain name line with a small greyed description under it (for balls, gold…).
func _named_with_desc(name_text: String, desc: String) -> Control:
	var box := VBoxContainer.new()
	box.add_theme_constant_override("separation", 0)
	box.add_child(_line(name_text))
	box.add_child(_desc_label(desc))
	return box

# Small, dimmed, wrapping description line shown under an item.
...
func _ball_desc(ball) -> String:
	if ball == &"master_ball":
		return "Catches any wild spirit automatically."
	return "Adds one winning number to your catch roll."
