...
# Trainer (character) roster — each ties to a starter and a colour. Only the
# four whose starters exist in the 20-spirit roster are offered.
const TRAINERS := [
	...
	{ "name": "Ash",  "starter": "Pikachu",    "color": Color("ffd24d") },
]
...
func _apply_trainer(card: Node) -> void:
	...
	if icon:
		icon.texture = SpiritsData.sprite_tex(trainer["starter"], "front")
